...
import {
  CommandExecutor,
  Commands,
} from './packages/zeroad-adapter/src/rl-interface/command-executor.js';
...
async function main() {
  ...
  try {
    ...
    const units =
      initialState.entities?.filter(e => e.type === 'unit').slice(0, 3) || [];
    const buildings =
      initialState.entities?.filter(e => e.type === 'building').slice(0, 1) ||
      [];
    ...
    if (units.length >= 1) {
      ...
      const stanceResult = await executor.executeCommand(
        1,
        stanceCmd,
        'SetStance'
      );
    ...
    if (units.length >= 1) {
      ...
      const cancelResult = await executor.executeCommand(
        1,
        cancelCmd,
        'CancelOrder'
      );
    ...
    if (units.length >= 2) {
      ...
      const batchResult: CommandTestResult = {
