...
test('BrainExecutor - retry rate tracking', async () => {
    const executor = new BrainExecutor({ decisionTimeoutMs: 5000, maxRetries: 3, retryDelayMs: 10, enableTelemetry: true }, logger);
    let attempts = 0;
    await executor.executeBrainDecision(async (token) => {
        attempts++;
        if (attempts < 2)
...
test('BrainExecutor - timeout rate tracking', async () => {
    const executor = new BrainExecutor({ decisionTimeoutMs: 50, maxRetries: 1, enableTelemetry: true }, logger);
    await executor.executeBrainDecision(async (token) => {
        await new Promise((resolve) => setTimeout(resolve, 200));
...
test('BrainExecutor - deterministic mode flag', async () => {
    const executor = new BrainExecutor({ decisionTimeoutMs: 5000, maxRetries: 1, enableDeterministicMode: false }, logger);
...
//# sourceMappingURL=brain-executor.test.js.map