...
test('ExternalSystemLifecycle - error threshold triggers degraded state', async () => {
    const lifecycle = new ExternalSystemLifecycle({ healthCheckIntervalMs: 0, errorThreshold: 3, errorWindowMs: 60000 }, logger);
    await lifecycle.initialize();
    assert.equal(lifecycle.getStatus(), ExternalSystemHealthStatus.Healthy);
...
test('ExternalSystemLifecycle - recovery from degraded state', async () => {
    const lifecycle = new ExternalSystemLifecycle({ healthCheckIntervalMs: 0, errorThreshold: 2, errorWindowMs: 60000, recoveryAttempts: 1, recoveryDelayMs: 10 }, logger);
    await lifecycle.initialize();
    lifecycle.recordError(new Error('Error 1'));
    lifecycle.recordError(new Error('Error 2'));
...
test('ExternalSystemLifecycle - recovery events', async () => {
    const lifecycle = new ExternalSystemLifecycle({ healthCheckIntervalMs: 0, errorThreshold: 1, recoveryAttempts: 1, recoveryDelayMs: 10 }, logger);
    await lifecycle.initialize();
...
test('ExternalSystemLifecycle - error window cleanup', async () => {
    const lifecycle = new ExternalSystemLifecycle({ healthCheckIntervalMs: 0, errorThreshold: 10, errorWindowMs: 50 }, logger);
    await lifecycle.initialize();
...
//# sourceMappingURL=external-system-lifecycle.test.js.map