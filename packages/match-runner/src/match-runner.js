...
export class MatchRunner {
    static async run(config) {
        ...
        for (let tick = 0; tick < config.maxTicks; tick++) {
            ...
            const redDecision = await config.redBrain.decide(currentRedState, redAvailableGoals, redAvailableCommands, redMemory);
            // Blue brain decision
            ...
            const blueDecision = await config.blueBrain.decide(currentBlueState, blueAvailableGoals, blueAvailableCommands, blueMemory);
        ...
        const redMetrics = config.redBrain.getMetrics?.() || {
            totalTokensUsed: 0,
            totalCost: 0,
        };
        const blueMetrics = config.blueBrain.getMetrics?.() || {
            totalTokensUsed: 0,
            totalCost: 0,
        };
        const metrics = {
    ...
    static createInitialWorldObservation(config, player) {
    ...
    static getAvailableGoals(state) {
        return [
            {
                id: 'gather-resources',
    ...
    static getAvailableCommands(state) {
        return [
            {
                id: 'move-north',
                action: 'move',
    ...
    static applyEffects(state, commands) {
        let newState = { ...state };
    ...
    static determineWinner(redState, blueState) {
...
//# sourceMappingURL=match-runner.js.map