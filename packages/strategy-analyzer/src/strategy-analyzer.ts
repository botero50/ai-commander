...
export class StrategyAnalyzer {
  static analyze(replay: MatchReplay, player: 'red' | 'blue'): StrategyProfile {
    ...
    const timeline: Array<{ tick: number; dominantGoal: string }> = [];
    ...
    for (const tick of replay.trace) {
      ...
      if (goal.includes('attack') || goal.includes('combat')) {
        rushCount += 1;
        if (tick.tickNumber < earlyGameTicks) earlyRushCount += 1;
  ...
  static compareStrategies(
    redProfile: StrategyProfile,
    blueProfile: StrategyProfile
  ): {
    matchup: string;
    ...
    const matchups: Record<Strategy, Record<Strategy, 'red' | 'blue' | 'neutral'>> = {
      'Rush': {
        'Rush': 'neutral',
        'Fast Expand': 'red',
        'Turtle': 'blue',
  ...
  private static explainMatchup(red: StrategyProfile, blue: StrategyProfile, advantage: string): string {
