...
export class CheckersGame {
  ...
  private redPieces = 12;
  private blackPieces = 12;
  private turn = 1; // 1 = red, -1 = black
  private moveHistory: Array<{ from: { x: number; y: number }; to: { x: number; y: number } }> = [];
  ...
  private executeCheckerMove(fromR: number, fromC: number, toR: number, toC: number) {
    ...
    if (Math.abs(toR - fromR) > 1 && this.board[midR] && this.board[midR][midC]) {
      ...
      if (capturedPiece > 0) this.redPieces--;
      if (capturedPiece < 0) this.blackPieces--;
    }
  }

  getWinner(): 'red' | 'black' | undefined {
    if (this.redPieces === 0) return 'black';
    if (this.blackPieces === 0) return 'red';
    return undefined;
  }
