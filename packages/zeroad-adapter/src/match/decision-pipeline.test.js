...
test('DecisionPipeline - retry rate tracking', async () => {
    const pipeline = new DecisionPipeline({ decisionTimeoutMs: 5000, maxRetries: 3, retryDelayMs: 10, enableTelemetry: true }, logger);
    let attempts = 0;
    await pipeline.executeDecision(async (token) => {
        attempts++;
        if (attempts < 2)
...
test('DecisionPipeline - timeout rate tracking', async () => {
    const pipeline = new DecisionPipeline({ decisionTimeoutMs: 50, maxRetries: 1, enableTelemetry: true }, logger);
    await pipeline.executeDecision(async (token) => {
        await new Promise((resolve) => setTimeout(resolve, 200));
...
//# sourceMappingURL=decision-pipeline.test.js.map