...
test('MatchLoop - brain lifecycle tracks uptime', async () => {
    ...
    const match = factory.createMatch({});
    await factory.startMatch(match);
    const loop = new MatchLoop(match, { tickDurationMs: 10, maxIterations: 1, brainLifecycle: { healthCheckIntervalMs: 0 } }, {}, logger, brain);
    const lifecycle = loop.getBrainLifecycle();
    await loop.start();
...
//# sourceMappingURL=match-loop-brain-lifecycle.test.js.map