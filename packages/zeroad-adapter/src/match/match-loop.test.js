...
test('MatchLoop - execute callback receives commands', async () => {
    ...
    const loop = new MatchLoop(match, { tickDurationMs: 10, maxIterations: 3 }, {
        onDecide: async () => {
            return [
                { id: 'cmd1', type: 'move', playerId: 1, timestamp: Date.now(), entityIds: [1], targetX: 100, targetZ: 100 },
                { id: 'cmd2', type: 'attack', playerId: 1, timestamp: Date.now(), entityIds: [1], targetEntityId: 2 },
            ];
        },
...
test('MatchLoop - max iterations stops loop', async () => {
    ...
    const match = factory.createMatch({ mapName: 'Egypt' });
    await factory.startMatch(match);
    const loop = new MatchLoop(match, { tickDurationMs: 10, maxIterations: 2 }, {}, logger);
    await loop.start();
    await new Promise(resolve => setTimeout(resolve, 100));
    assert.equal(loop.isRunning(), false);
    await match.stop();
...
test('MatchLoop - collects performance metrics', async () => {
    ...
    const match = factory.createMatch({ mapName: 'Egypt' });
    await factory.startMatch(match);
    const loop = new MatchLoop(match, { tickDurationMs: 5, maxIterations: 5 }, {
        onObserve: async () => {
...
//# sourceMappingURL=match-loop.test.js.map