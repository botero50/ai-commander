...
test('BrainIntegrationValidator - builtin brain integration', async () => {
    ...
    let tick = 0;
    const result = await validator.validateIntegrationCycle(brain, async () => createMockWorldState(tick++), async (state) => [
...
test('BrainIntegrationValidator - llm brain integration', async () => {
    ...
    let tick = 0;
    const result = await validator.validateIntegrationCycle(brain, async () => createMockWorldState(tick++), async (state) => [{ id: 'goal-act', intent: 'Act intelligently', priority: 'high', feasibility: 1, expectedDuration: 5000, estimatedValue: 100 }], async (state) => [], async (commands) => {
...
test('BrainIntegrationValidator - cycle validation results', async () => {
    ...
    const brain = new MockBuiltinBrain();
    let tick = 0;
    const result = await validator.validateIntegrationCycle(brain, async () => createMockWorldState(tick++), async () => [], async () => [], async () => { }, 2);
...
test('BrainIntegrationValidator - determinism verification', async () => {
    ...
    const brain = new MockBuiltinBrain();
    let tick = 0;
    const result = await validator.validateIntegrationCycle(brain, async () => createMockWorldState(tick++), async () => [], async () => [], async () => { }, 5);
...
test('BrainIntegrationValidator - command execution tracking', async () => {
    ...
    const result = await validator.validateIntegrationCycle(brain, async () => createMockWorldState(tick++), async () => [], async () => [], async (commands) => {
        executeCallCount++;
    }, 2);
...
test('BrainIntegrationValidator - latency measurement', async () => {
    ...
    const brain = new MockLLMBrain();
    let tick = 0;
    const result = await validator.validateIntegrationCycle(brain, async () => createMockWorldState(tick++), async () => [], async () => [], async () => { }, 3);
    assert(result.metrics.avgDecisionLatencyMs > 0);
    assert(result.metrics.avgCycleLatencyMs > 0);
...
//# sourceMappingURL=brain-integration-validator.test.js.map