...
export async function runSimpleMatch(session, brain, config = {}) {
    ...
    const gameLoop = new GameLoop(session, {
        tickDurationMs: 50,
        ...
        onDecide: async (state) => {
            try {
                ...
                const decision = await brain.decide(state, [], // availableGoals (empty for now)
 ...
 */
export async function runDualBrainMatch(session, brain1, brain2, config = {}) {
    ...
    const gameLoop = new GameLoop(session, {
        ...
        onDecide: async (state) => {
            ...
            try {
                const startTime = Date.now();
                const decision = await brain.decide(state, [], // availableGoals
                [], // availableCommands
                {
                    // ExecutionMemory
                    recentEvents: [],
                ...
                if (matchConfig.decisionOverlay) {
                    matchConfig.decisionOverlay.recordDecision(tickCount, player, brain.name, decision.reasoning, decision.commands || [], durationMs);
                }
