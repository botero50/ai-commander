...
// Local type definitions (no SDK imports)
interface Brain {
...
test('MatchLoop + Brain - Brain receives immutable WorldState', async () => {
  ...
  class ImmutabilityCheckBrain implements Brain {
    readonly name = 'test-brain';
    readonly version = '1.0.0';

    async decide(observation: any): Promise<BrainDecision> {
...
test('MatchLoop + Brain - multiple iterations use Brain', async () => {
  ...
  const loop = new MatchLoop(
    match,
    { tickDurationMs: 10, maxIterations: 5 },
    {},
    logger,
    brain
  );
