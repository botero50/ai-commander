...
test('BrainLifecycle - error threshold triggers degraded state', async () => {
    const lifecycle = new BrainLifecycle({ healthCheckIntervalMs: 0, errorThreshold: 3, errorWindowMs: 60000 }, logger);
    await lifecycle.initialize();
    assert.equal(lifecycle.getStatus(), BrainHealthStatus.Healthy);
...
test('BrainLifecycle - recovery from degraded state', async () => {
    const lifecycle = new BrainLifecycle({ healthCheckIntervalMs: 0, errorThreshold: 2, errorWindowMs: 60000, recoveryAttempts: 1, recoveryDelayMs: 10 }, logger);
    await lifecycle.initialize();
...
test('BrainLifecycle - recovery failure transitions to failed', async () => {
    const lifecycle = new BrainLifecycle({ healthCheckIntervalMs: 0, errorThreshold: 1, recoveryAttempts: 1, recoveryDelayMs: 0 }, logger);
    await lifecycle.initialize();
    lifecycle.recordError(new Error('Error 1'));
    await lifecycle.performHealthCheck();
    assert.equal(lifecycle.getStatus(), BrainHealthStatus.Degraded);
...
test('BrainLifecycle - recovery events', async () => {
    const lifecycle = new BrainLifecycle({ healthCheckIntervalMs: 0, errorThreshold: 1, recoveryAttempts: 1, recoveryDelayMs: 10 }, logger);
    await lifecycle.initialize();
...
test('BrainLifecycle - error window cleanup', async () => {
    const lifecycle = new BrainLifecycle({ healthCheckIntervalMs: 0, errorThreshold: 10, errorWindowMs: 50 }, logger);
    await lifecycle.initialize();
...
//# sourceMappingURL=brain-lifecycle.test.js.map