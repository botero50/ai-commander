 ...
 */
export function parseTournamentOptions(args) {
    let format = 'round_robin';
    const brains = ['Ollama', 'Ollama'];
 ...
 */
export function showTournamentHelp() {
 ...
 */
export async function tournamentRunCommand(args) {
    ...
    const brainConfig = options.brains.map((name, i) => ({
        id: `brain-${i}`,
        name,
        version: '1.0.0',
        brain: null, // Would be real brain in production
    }));
    const config = {
        name: options.name,
        brains: brainConfig,
    ...
    try {
        ...
        for (let i = 0; i < Math.min(5, rankings.length); i++) {
            ...
            console.log(`${i + 1}. ${rank.name.padEnd(20)} | Wins: ${rank.wins} | Losses: ${rank.losses} | Draw: ${rank.draws}`);
        }
        return 0;
    }
    catch (err) {
 ...
 */
export async function tournamentStatusCommand(args) {
    if (args.length === 0) {
 ...
 */
export async function tournamentListCommand(args) {
