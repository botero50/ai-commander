...
import { StatisticsAnalyzer, StatisticsSnapshot } from './statistics-analyzer.js';
...
export interface PlayerPerformancePrediction {
  playerId: number;
  predictedEconomyScore: number; // 1-10
...
export interface TurningPointPrediction {
  timeUntilTurningPoint: number; // seconds
  predictedEventType: string; // "military_clash", "expansion_completion", "tech_unlock", "economic_shift"
...
export class PredictionSystem {
  ...
  private comparisonEngine: MatchComparisonEngine;
  private models: Map<string, PredictionModel> = new Map();
  private historicalSnapshots: StatisticsSnapshot[] = [];
  private predictions: MatchOutcomePrediction[] = [];

  constructor(comparisonEngine?: MatchComparisonEngine) {
    this.statisticsAnalyzer = new StatisticsAnalyzer();
    this.comparisonEngine = comparisonEngine || new MatchComparisonEngine();
  ...
  predictPlayerPerformance(playerId: number, state: GameState): PlayerPerformancePrediction {
    ...
    if (lastSnapshot.economy.economyScore > 6) strengths.push('Strong economy');
    if (lastSnapshot.economy.economyScore < 4) weaknesses.push('Weak economy');

    if (lastSnapshot.military.militaryScore > 6) strengths.push('Strong military');
    if (lastSnapshot.military.militaryScore < 4) weaknesses.push('Weak military');

    if (lastSnapshot.tech.techsUnlocked > 3) strengths.push('Advanced technology');
    if (lastSnapshot.activity.activityScore > 6) strengths.push('High activity level');
    if (lastSnapshot.activity.activityScore < 4) weaknesses.push('Low activity level');
   ...
   */
  private calculatePlayerScore(snapshot: StatisticsSnapshot): number {
   ...
   */
  private calculateTrend(startValue: number, endValue: number): string {
    if (endValue > startValue + 0.5) return 'growing';
    if (endValue < startValue - 0.5) return 'declining';
    return 'stable';
  }
   ...
   */
  private getDefaultPrediction(): MatchOutcomePrediction {
    return {
      predictedWinner: 1,
      confidence: 0.5,
  ...
  private getDefaultStrategyPrediction(): StrategyPrediction {
    return {
  ...
  private getDefaultPerformancePrediction(playerId: number): PlayerPerformancePrediction {
    return {
      playerId,
  ...
  private getDefaultTurningPointPrediction(): TurningPointPrediction {
    return {
