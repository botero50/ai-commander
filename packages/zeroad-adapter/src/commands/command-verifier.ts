...
export class CommandVerifier {
  ...
  verify(command: GameCommand, currentState: GameState, previousState: GameState | null): VerificationResult {
    ...
    try {
      ...
      switch (command.type) {
        case 'move':
          [verified, evidence] = this.verifyMove(command as any, currentState, previousState);
          break;
        case 'attack':
          [verified, evidence] = this.verifyAttack(command as any, currentState, previousState);
          break;
        case 'gather':
          [verified, evidence] = this.verifyGather(command as any, currentState, previousState);
          break;
        case 'build':
          [verified, evidence] = this.verifyBuild(command as any, currentState, previousState);
          break;
        case 'train':
          [verified, evidence] = this.verifyTrain(command as any, currentState, previousState);
          break;
        case 'patrol':
          [verified, evidence] = this.verifyPatrol(command as any, currentState, previousState);
          break;
        case 'repair':
          [verified, evidence] = this.verifyRepair(command as any, currentState, previousState);
          break;
        case 'stop':
          [verified, evidence] = this.verifyStop(command as any, currentState, previousState);
          break;
        default:
  ...
  private verifyMove(cmd: any, current: GameState, previous: GameState): [boolean, string] {
    // Verify that at least one unit has moved closer to target or reached it
    const unitIds = cmd.entityIds as number[];
    ...
    for (const unitId of unitIds) {
      ...
      if (!currentUnit || !previousUnit) continue;
  ...
  private verifyAttack(cmd: any, current: GameState, previous: GameState): [boolean, string] {
    // Verify that units have attack orders or target entity shows damage
    const unitIds = cmd.entityIds as number[];
    const targetId = cmd.targetEntityId as number;
  ...
  private verifyGather(cmd: any, current: GameState, previous: GameState): [boolean, string] {
  ...
  private verifyBuild(cmd: any, current: GameState, previous: GameState): [boolean, string] {
    // Verify that a new building was created or construction started
    const posX = cmd.positionX as number;
    const posZ = cmd.positionZ as number;
    const template = cmd.templateName as string;
    ...
    if (currentBuildingCount > previousBuildingCount) {
      const newBuilding = currentBuildings.find(
        (b) => b.position.x === posX && b.position.z === posZ && b.type === template
      );
      if (newBuilding) {
  ...
  private verifyTrain(cmd: any, current: GameState, previous: GameState): [boolean, string] {
    // Verify that building has units in production queue
    const builderId = cmd.builderEntityId as number;
    const template = cmd.templateName as string;
  ...
  private verifyPatrol(cmd: any, current: GameState, previous: GameState): [boolean, string] {
  ...
  private verifyRepair(cmd: any, current: GameState, previous: GameState): [boolean, string] {
  ...
  private verifyStop(cmd: any, current: GameState, previous: GameState): [boolean, string] {
  ...
  private distance(pos1: { x: number; z: number }, pos2: { x: number; z: number }): number {
