...
export class ReplayExport {
     ...
     */
    static toJSON(metadata, decisions, snapshots, options = {}) {
     ...
     */
    static toCSV(metadata, decisions) {
        const lines = [];
        // Header
        ...
        for (const decision of decisions) {
            ...
            lines.push(`${decision.tick},${player},${this.escapeCSV(decision.brainName)},${decision.commandCount},0,${decision.durationMs},"${this.escapeCSV(decision.reasoning || '')}"` // eslint-disable-line no-unsafe-optional-chaining
            );
        }
        return lines.join('\n');
    }
     ...
     */
    static toHTML(metadata, decisions, snapshots) {
        const p1ErrorRate = metadata.player1Commands > 0
            ? ((metadata.player1Errors / metadata.player1Commands) * 100).toFixed(2)
            : '0.00';
        const p2ErrorRate = metadata.player2Commands > 0
...
//# sourceMappingURL=replay-export.js.map