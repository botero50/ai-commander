...
export class TrendAnalyzer {
  ...
  ): Record<number, { economy: string; military: string; tech: string }> {
  ...
  generateTrendReport(stats: StatisticsSnapshot[]): string {
    ...
    const economyTrend = this.getTrendDescription(
      first.economy.economyScore,
      last.economy.economyScore,
      'economy'
    );
    const militaryTrend = this.getTrendDescription(
      first.military.militaryScore,
      last.military.militaryScore,
      'military'
    );
    const techTrend = this.getTrendDescription(
      first.tech.techsUnlocked,
      last.tech.techsUnlocked,
      'tech progression'
    );
