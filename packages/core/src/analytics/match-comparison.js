...
exports.MatchComparisonEngine = void 0;
/**
 * Compares multiple matches to find patterns
 */
class MatchComparisonEngine {
    matchHistories = new Map();
    playerProfiles = new Map();
    /**
     * Add match to comparison pool
     */
    addMatch(matchId, stats, winner) {
     ...
     */
    createPlayerProfile(matchId, playerId, snapshots) {
        ...
        // Calculate rates
        const techAdvanceRate = snapshots.length > 1
    ...
    generateInsights(matches) {
        ...
        const avgWinnerEconomy = matches.reduce((sum, m) => sum + m.economyScore, 0) / Math.max(1, matches.length);
        const avgWinnerMilitary = matches.reduce((sum, m) => sum + m.militaryScore, 0) / Math.max(1, matches.length);
     ...
     */
    analyzeWinnerCharacteristics(matches) {
        if (matches.length === 0) {
     ...
     */
    analyzeLoserCharacteristics(matches) {
        if (matches.length === 0) {
            return { avgEconomy: 0, avgMilitary: 0, avgTech: 0, commonStrategies: [] };
        }
        ...
        const commonStrategies = [];
        if (avgEconomy < 4)
            commonStrategies.push('economy_neglect');
        if (avgMilitary < 4)
            commonStrategies.push('military_weakness');
        if (avgTech < 4)
            commonStrategies.push('tech_delay');
        if (commonStrategies.length === 0)
            commonStrategies.push('insufficient_advantage');
        return { avgEconomy, avgMilitary, avgTech, commonStrategies };
    }
    /**
     * Get player statistics across matches
     */
    getPlayerComparison(playerId) {
        ...
        // Calculate averages
        const avgEconomy = Math.round((profiles.reduce((sum, p) => sum + p.economyProgression[p.economyProgression.length - 1], 0) / profiles.length) *
            10) / 10;
        const avgMilitary = Math.round((profiles.reduce((sum, p) => sum + p.militaryProgression[p.militaryProgression.length - 1], 0) / profiles.length) *
            10) / 10;
        ...
        if (avgEconomy > avgMilitary)
            preferredStyle = 'economic';
        if (avgMilitary > avgEconomy)
            preferredStyle = 'military';
        if (avgTechRate > 0.5)
            preferredStyle = 'tech_heavy';
        return {
...
exports.MatchComparisonEngine = MatchComparisonEngine;
//# sourceMappingURL=match-comparison.js.map