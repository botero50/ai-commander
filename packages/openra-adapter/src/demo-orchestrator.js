...
export class DemoOrchestrator {
    ...
    static async run(config) {
        ...
        if (!configA || !configB) {
            console.error(`❌ Unknown brain: ${!configA ? config.brainA : config.brainB}`);
     ...
     * Run single match.
     */
    static async runSingleMatch(configA, configB, config) {
        ...
        if (config.exportPath) {
            ...
            await fs.writeFile(`${config.exportPath}/report.json`, BenchmarkReporter.exportJSON(report));
     ...
     */
    static async runMultiMatch(configA, configB, config) {
        console.log(`Running ${config.games} matches: ${config.brainA} vs ${config.brainB}\n`);
        // Progress indicator
        ...
        const report = BenchmarkReporter.reportMultiMatch(result, config.brainA, config.brainB);
        console.log(report.summary);
        console.log("\n" + report.results);
        console.log("\n" + report.analysis);
        // Cost analysis
        ...
        console.log("\n🎯 Strategy Analysis:");
        const strategies = [
            StrategyAnalyzer.analyzeProviderStrategy(config.brainA, result, result.stats.provider1Wins, result.stats.provider2Wins),
            StrategyAnalyzer.analyzeProviderStrategy(config.brainB, result, result.stats.provider2Wins, result.stats.provider1Wins),
        ];
...
//# sourceMappingURL=demo-orchestrator.js.map