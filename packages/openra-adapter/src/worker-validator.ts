...
export interface WorkerValidationResult {
  ...
  readonly cycleTime: number; // Ticks to complete one gather cycle
}
...
export class WorkerValidator {
   ...
   * 4. Deposits
   */
  static validateWorkerCycle(
    startState: OpenRAGameState,
    finalState: OpenRAGameState,
    events: GameEvent[],
    playerName: string
    ...
    // Find a Harvester
    const harvester = startState.units.find(
    ...
    const refinery = startState.buildings.find(
    ...
    const finalHarvester = finalState.units.find(
      (u) => u.id === harvester.id
    );
    ...
    if (!finalHarvester) {
      ...
      if (!returned) {
        errors.push(
          `Harvester not returned to refinery (distance: ${distanceToRefinery})`
        );
      }
    }

    // Phase 4: Deposit resources
    const finalPlayerResources = finalState.players.find(
      (p) => p.name === playerName
    )?.credits || 0;

    const startPlayerResources = startState.players.find(
      (p) => p.name === playerName
    )?.credits || 0;
  ...
  static validateMultipleCycles(
    ...
    const cycleResults: WorkerValidationResult[] = [];
    ...
    for (let i = 0; i < Math.min(cycles, states.length - 1); i++) {
      ...
      const result = this.validateWorkerCycle(
        startState,
        endState,
        cycleEvents,
        playerName
