...
export class MultiMatchRunner {
     ...
     */
    static async runMatches(config) {
        const matches = [];
    ...
    static aggregateStats(matches) {
        ...
        const avgResources1 = matches.reduce((sum, m) => sum + m.stats.provider1Resources, 0) / totalMatches;
        const avgResources2 = matches.reduce((sum, m) => sum + m.stats.provider2Resources, 0) / totalMatches;
        ...
        const avgBuildings1 = matches.reduce((sum, m) => sum + m.stats.provider1Buildings, 0) / totalMatches;
        const avgBuildings2 = matches.reduce((sum, m) => sum + m.stats.provider2Buildings, 0) / totalMatches;
        // Variance
        const varianceResources1 = matches.reduce((sum, m) => sum + Math.pow(m.stats.provider1Resources - avgResources1, 2), 0) /
            totalMatches;
        const varianceResources2 = matches.reduce((sum, m) => sum + Math.pow(m.stats.provider2Resources - avgResources2, 2), 0) /
            totalMatches;
...
//# sourceMappingURL=multi-match-runner.js.map