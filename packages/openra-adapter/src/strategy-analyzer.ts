...
export interface StrategyMetrics {
  ...
  readonly aggressiveness: number; // 0-100 (unit production rate)
  readonly economyFocus: number; // 0-100 (harvester/refinery ratio)
  readonly defensiveness: number; // 0-100 (building preservation)
}
...
export class StrategyAnalyzer {
   ...
   */
  static analyzeFromMatch(
    result: MatchResult,
    playerStats: { resourcesGathered: number; unitsProduced: number; buildingsConstructed: number }
  ): StrategyMetrics {
    ...
    const strategy: Strategy = isBalanced
      ? "balanced"
   ...
   */
  static analyzeProviderStrategy(
    provider: string,
    results: MultiMatchResult,
    wins: number,
    losses: number
  ): ProviderStrategy {
  ...
  static compareStrategies(
    provider1: ProviderStrategy,
    provider2: ProviderStrategy
  ): { description: string; advantages: Map<string, string[]> } {
    const advantages = new Map<string, string[]>();
...
${provider2.provider} (${p2.strategy}): Focus on ${p2.strategy}`;
  ...
  static generateReport(strategies: ProviderStrategy[]): string {
    ...
    for (const ps of strategies) {
      const s = ps.strategy;
      lines.push(
        `${ps.provider.padEnd(8)} | ${s.strategy.padEnd(8)} | ${s.score.toFixed(0).padEnd(5)} | ${s.aggressiveness.toFixed(0).padEnd(10)} | ${s.economyFocus.toFixed(0).padEnd(7)} | ${s.defensiveness.toFixed(0).padEnd(9)} | ${ps.wins}-${ps.losses}`
      );
    }
