...
import type { OpenRAGameState, OpenRABuilding } from "./state-reader";
...
export class WorldMapper {
  static mapToObservation(gameState: OpenRAGameState, playerName: string): WorldObservation {
    ...
    const mainStructure = friendlyBuildings.find((b) =>
      ["ConYard", "BarracksN", "WarFactory"].includes(b.type)
    );
  ...
  private static calculatePlayerHealth(buildings: OpenRABuilding[]): number {
