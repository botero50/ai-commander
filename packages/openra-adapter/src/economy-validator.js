...
export class EconomyValidator {
     ...
     */
    static validateEconomy(startState, endState, events, playerName) {
        ...
        const harvesterCreatedEvents = events.filter((e) => e.type === "unit-created" &&
        ...
        const buildingCompletedEvents = events.filter((e) => e.type === "building-completed" && !e.detail.destroyed);
        ...
        const resourceGatheredEvents = events.filter((e) => e.type === "resource-gathered");
     ...
     */
    static validateSustainedEconomy(states, allEvents, playerName, samplePeriods = 3) {
        const results = [];
        ...
        for (let i = 0; i < states.length - stateInterval; i += stateInterval) {
            ...
            const periodEvents = allEvents.filter((e) => e.tick >= startState.tick && e.tick <= endState.tick);
            const result = this.validateEconomy(startState, endState, periodEvents, playerName);
            results.push(result);
        }
    ...
    static calculateMetrics(state, tick, playerName) {
        const harvesters = state.units.filter((u) => u.type === "Harvester" && u.owner === playerName);
        const refineries = state.buildings.filter((b) => b.type === "Refinery" && b.owner === playerName);
...
//# sourceMappingURL=economy-validator.js.map