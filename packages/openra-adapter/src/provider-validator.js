...
export class ProviderValidator {
     ...
     */
    static async validateProviders(brains, gameState, playerNames, events) {
        const errors = [];
        const observations = new Map();
        const decisions = new Map();
        // Generate observations for each player
        const obsPerPlayer = new Map();
        const goalsPerPlayer = new Map();
        const commandsPerPlayer = new Map();
     ...
     */
    static obsEqual(obs1, obs2) {
        // Compare key fields
        if (obs1.tick !== obs2.tick)
            return false;
        if (obs1.missionId !== obs2.missionId)
            return false;
        if (obs1.friendlyUnits.length !== obs2.friendlyUnits.length)
            return false;
        if (obs1.enemyUnits.length !== obs2.enemyUnits.length)
            return false;
        if (obs1.agentHealth !== obs2.agentHealth)
            return false;
