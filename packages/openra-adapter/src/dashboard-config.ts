...
export class DashboardConfig {
  ...
  static costWidget(costAnalysis: TournamentCostAnalysis): DashboardWidget {
    const providers = Array.from(costAnalysis.providers.values()).sort(
  ...
  static generateHTML(dashboard: DashboardData): string {
    const standings = dashboard.tournament.standings
      .map(
        (s, i) =>
          `<tr><td>${i + 1}</td><td>${s.provider}</td><td>${s.wins}</td><td>${s.losses}</td><td>${s.draws}</td><td>${s.points}</td></tr>`
      )
      .join("");
    ...
    const ratings = Array.from(dashboard.ratings.values())
      ...
      .map(
        (r, i) =>
          `<tr><td>${i + 1}</td><td>${r.provider}</td><td>${r.current.toFixed(0)}</td><td>${r.matches}</td></tr>`
      )
      .join("");
