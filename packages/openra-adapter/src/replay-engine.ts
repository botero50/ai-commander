...
export interface ReplayComparison {
  ...
  readonly similarities: number; // Percentage of identical ticks
...
export class ReplayEngine {
  private static readonly MAX_REPLAY_SIZE = 10_000_000; // 10MB max
  ...
  static recordReplay(
    ...
    if (size > this.MAX_REPLAY_SIZE) {
      console.warn(
        `Replay size ${(size / 1_000_000).toFixed(1)}MB exceeds limit, may be truncated`
      );
    }
  ...
  static compareReplays(replay1: Replay, replay2: Replay): ReplayComparison {
    const differences: Array<{ tick: number; description: string }> = [];
    const minTicks = Math.min(replay1.totalTicks, replay2.totalTicks);

    let divergeAtTick = -1;
    let divergeEvent: GameEvent | null = null;
   ...
   */
  private static statesEqual(state1: OpenRAGameState, state2: OpenRAGameState): boolean {
    // Compare key fields
    if (state1.tick !== state2.tick) return false;
    if (state1.units.length !== state2.units.length) return false;
    if (state1.buildings.length !== state2.buildings.length) return false;

    // Compare player resources
    for (let i = 0; i < state1.players.length && i < state2.players.length; i++) {
      if (state1.players[i].credits !== state2.players[i].credits) return false;
    }
    ...
    for (const u1 of state1.units) {
      ...
      if (!u2) return false;
      if (u1.health !== u2.health || u1.x !== u2.x || u1.y !== u2.y) return false;
    }
  ...
  static generateComparisonReport(comparison: ReplayComparison): string {
    ...
    if (comparison.divergeEvent) {
      ...
      lines.push(
        `  Tick ${comparison.divergeEvent.tick}: ${comparison.divergeEvent.type}`
      );
