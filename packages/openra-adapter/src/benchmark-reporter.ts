...
Draws: ${stats.draws}`;
   ...
   */
  static reportTournament(
    result: TournamentResult,
    costAnalysis?: TournamentCostAnalysis
  ): BenchmarkReport {
    const timestamp = new Date().toISOString();
...
${costAnalysis ? `Total cost: \$${costAnalysis.totalCost.toFixed(2)}` : ""}`;
   ...
   */
  private static formatSingleMatchResults(result: SingleMatchResult): string {
    const lines = [
      "=== Match Results ===",
   ...
   */
  private static formatMultiMatchResults(result: MultiMatchResult): string {
    const stats = result.stats;
    const lines = [
      "=== Multi-Match Results ===",
   ...
   */
  private static formatTournamentResults(result: TournamentResult): string {
    const lines = [
   ...
   */
  private static analyzeSingleMatch(result: SingleMatchResult): string {
    const lines = [
      "=== Gameplay Analysis ===",
      `Validation: ${result.validationPassed ? "PASS" : "FAIL"}`,
   ...
   */
  private static analyzeMultiMatch(result: MultiMatchResult): string {
    const stats = result.stats;
    const lines = [
      "=== Trend Analysis ===",
  ...
  private static analyzeTournament(
    ...
    if (costAnalysis) {
      ...
      const roiData = result.standings
        .map((s) => ({
          provider: s.provider,
          roi: CostAnalyzer.calculateROI(
            s.provider as any,
            costAnalysis.providers.get(s.provider)?.totalCost || 0,
            s.wins,
            s.totalGames
          ),
        }))
