...
export class BenchmarkReporter {
    static generateReport(result) {
        const metrics = result.standings.map((standing) => {
            const brainMatches = result.matches.filter((m) => m.metrics.redPlayer === standing.brainName || m.metrics.bluePlayer === standing.brainName);
            ...
            const avgDecisionTime = brainMatches.length > 0
            ...
            return {
                ...
                avgResourcesPerGame: totalResources / (standing.wins + standing.losses + standing.draws),
            };
        });
        return {
            timestamp: Date.now(),
...
`;
    ...
    static toCSV(report) {
        const headers = 'Brain,Wins,Losses,Draws,Win Rate %,ELO,Avg Decision Time ms,Total Tokens,Total Cost USD,Cost Per Game USD,Resources Gathered,Resources Per Game';
        const rows = report.metrics
            .map((m) => {
