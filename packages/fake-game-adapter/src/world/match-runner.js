...
export class MatchRunner {
    ...
    async runMatch() {
        ...
        for (let tick = 1; tick <= this.config.maxTicks; tick++) {
            ...
            const p1Resources = world.players?.[0]?.resources || 0;
            const p2Resources = world.players?.[1]?.resources || 0;
     ...
     */
    async makeDecision(world, brain, player, tick) {
        const startTime = performance.now();
        ...
        const input = {
            ...
            memory: {
                previousDecisions: Object.freeze(this.decisions.filter((d) => d.player === player).slice(-5).map(d => ({
                    goal: d.goal,
                    commands: [],
                    tick: d.tick,
                    outcome: 'executed'
