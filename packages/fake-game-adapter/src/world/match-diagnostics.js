 ...
 */
export function analyzeMatch(world) {
    ...
    const workerEfficiency = diag.workersProduced > 0 ? (diag.workersProduced / (diag.workersProduced * 50)) * 100 : 0;
    const militaryEfficiency = diag.militaryTrained > 0 ? (diag.militaryTrained / (diag.militaryTrained * 100)) * 100 : 0;
    const combatEfficiency = diag.peakMilitaryCount > 0 ? (diag.enemiesKilled / diag.peakMilitaryCount) * 100 : 0;
    return {
        gameWon: isWon,
        gameLost: isLost,
 ...
 */
function analyzeFailure(world) {
    const diag = world.diagnostics;
    const reason = diag.failureReason || 'unknown';
    const tick = diag.failureTick || world.tick;
    switch (reason) {
 ...
 */
function analyzeNoUnitsFailure(world) {
    const diag = world.diagnostics;
    const tick = diag.failureTick || world.tick;
 ...
 */
function analyzeArmyDefeatFailure(world) {
    const diag = world.diagnostics;
    const tick = diag.failureTick || world.tick;
 ...
 */
function analyzeResourceAccessFailure(world) {
    const diag = world.diagnostics;
    const tick = diag.failureTick || world.tick;
 ...
 */
function analyzeEconomyFailure(world) {
    const diag = world.diagnostics;
    const tick = diag.failureTick || world.tick;
 ...
 */
export function generateDiagnosticReport(world) {
...
//# sourceMappingURL=match-diagnostics.js.map