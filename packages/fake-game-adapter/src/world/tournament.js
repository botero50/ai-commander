 ...
 */
export function calculateEloChange(playerRating, opponentRating, outcome, kFactor = 32) {
 ...
 */
export function createInitialRating(model) {
 ...
 */
export function updatePlayerRating(currentRating, outcome, opponentRating) {
 ...
 */
export function recordTournamentMatch(match, standings) {
    ...
    let player1Outcome = match.outcome;
    let player2Outcome = match.outcome === 'win' ? 'loss' : match.outcome === 'loss' ? 'win' : 'draw';
    // Update ratings
 ...
 */
export function generateRoundRobinSchedule(models) {
    const schedule = [];
 ...
 */
export function createTournament(tournamentId, models) {
 ...
 */
export function generateLeaderboard(standings) {
 ...
 */
export function isTournamentComplete(standings, scheduledMatchCount) {
    return standings.matches.length >= scheduledMatchCount;
}
/**
 * Generate tournament report
 */
export function generateTournamentReport(standings) {
    ...
    recentMatches.forEach((match, index) => {
        const result = match.outcome === 'win'
