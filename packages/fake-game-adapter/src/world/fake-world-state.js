 ...
 */
export function progressTick(world) {
    return Object.freeze({
        ...world,
        tick: world.tick + 1,
    });
 ...
 */
export function moveWorker(world, workerId, dx, dy) {
 ...
 */
export function waitWorker(world, _workerId) {
    return Object.freeze({
        ...world,
        commandsExecuted: world.commandsExecuted + 1,
    });
 ...
 */
export function gatherWorker(world, workerId) {
    const workers = Array.from(world.workers);
    const workerIndex = workers.findIndex((w) => w.id === workerId);
    if (workerIndex === -1) {
        return world;
 ...
 */
export function depositWorker(world, workerId) {
    const workers = Array.from(world.workers);
    const workerIndex = workers.findIndex((w) => w.id === workerId);
    if (workerIndex === -1) {
        return world;
 ...
 * Costs 50 resources.
 */
export function produceWorker(world) {
    const workerCost = 50;
 ...
 */
export function trainMilitaryUnit(world, unitType = 'infantry') {
 ...
 */
export function scoutArea(world, unitId) {
    ...
    for (const enemy of newDetections) {
        ...
        if (!existing) {
            knownEnemies.push(Object.freeze({
                unitId: enemy.id,
                x: enemy.x,
                y: enemy.y,
                lastSeen: world.tick,
 ...
 */
export function moveMilitaryUnit(world, unitId, dx, dy) {
    const units = Array.from(world.militaryUnits);
 ...
 */
export function attackUnit(world, attackerId, targetId) {
 ...
 */
export function checkDefeat(world) {
    if (world.gameState !== 'playing') {
        return world; // Already won or lost
    }
    if (world.workers.length === 0 && world.militaryUnits.length === 0) {
        const newDiagnostics = Object.freeze({
            ...world.diagnostics,
            failureReason: 'no-workers-no-military',
...
//# sourceMappingURL=fake-world-state.js.map