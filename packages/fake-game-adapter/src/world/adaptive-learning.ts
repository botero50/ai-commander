...
export interface LearningMetrics {
  ...
  readonly learningRate: number; // How quickly to adapt
  readonly confidenceLevel: number; // 0-100, based on sample size
}
...
export class AdaptiveLearningSystem {
  private strategies = new Map<string, StrategyOutcome>();
  private opponents = new Map<string, OpponentPattern>();
  ...
  private adaptationHistory: AdaptationRecommendation[] = [];
  ...
  adaptTactic(opponentId: string): AdaptationRecommendation {
    ...
    const recommendation_obj: AdaptationRecommendation = {
      recommendation,
      rationale,
      expectedWinRateImprovement: improvement,
