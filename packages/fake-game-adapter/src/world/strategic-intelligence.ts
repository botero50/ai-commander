...
export interface ThreatAssessment {
  ...
  readonly playerAdvantage: number; // Positive = player ahead
...
export class StrategicIntelligence {
  ...
  assessThreat(world: FakeWorldSnapshot): ThreatAssessment {
    ...
    for (const enemy of world.enemyUnits) {
      const type = enemy.type as 'infantry' | 'ranged' | 'tank';
    ...
    for (const unit of world.militaryUnits) {
      const type = unit.type as 'infantry' | 'ranged' | 'tank';
      const damage = type === 'infantry' ? 10 : type === 'ranged' ? 15 : 20;
      playerDamage += damage;
    }
  ...
  determinePriority(world: FakeWorldSnapshot): ResourcePriority {
    ...
    let phase: ResourcePriority['phase'];
    const priorityList: ResourceAllocation[] = [];
  ...
  planPositioning(world: FakeWorldSnapshot): PositioningStrategy {
    const baseX = 10;
    const baseY = 10;
    const baseDefensePositions: UnitPosition[] = [];
    const scoutPatrol: UnitPosition[] = [];
    const attackFormation: UnitPosition[] = [];
    const gatheringPattern: UnitPosition[] = [];
  ...
  analyzeBehaviorPattern(world: FakeWorldSnapshot, previousWorlds?: FakeWorldSnapshot[]): BehaviorPattern {
    ...
    let type: BehaviorPattern['type'] = 'mixed';
    ...
    if (enemyMovement > 5) {
      ...
      if (type !== 'aggressive') type = 'aggressive';
    }
  ...
  private analyzeMovement(current: FakeWorldSnapshot, previous: FakeWorldSnapshot): number {
  ...
  recordDecision(decision: Omit<StrategicDecision, 'timestamp'>): StrategicDecision {
    const fullDecision: StrategicDecision = {
      ...decision,
      timestamp: Date.now(),
    };
