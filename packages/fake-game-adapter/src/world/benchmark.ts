...
import { analyzeMatch, type MatchAnalysis } from './match-diagnostics.js';
 ...
 */
export function aggregateModelBenchmark(
  model: LLMModel,
 ...
 */
export function compileBenchmarkSuite(
  models: ReadonlyArray<LLMModel>,
  results: ReadonlyArray<BenchmarkResult>
): BenchmarkSuite {
  const benchmarks = new Map<LLMModel, ModelBenchmark>();
  let totalMatches = 0;
 ...
 */
function generateBenchmarkSummary(benchmarks: Map<LLMModel, ModelBenchmark>): BenchmarkSummary {
  ...
  const scores = new Map<LLMModel, number>();
