...
export class CLIInterface {
  ...
  static parseArgs(args: string[]): Partial<CLIConfig> {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '--format') {
        config.tournamentFormat = args[++i] as any;
      } else if (arg === '--models') {
   ...
   */
  static validateConfig(config: CLIConfig): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
...
`;
   ...
   */
  static formatOutput(result: CLIResult, verbose: boolean = false): string {
    let output = '';
   ...
   */
  static createProgressReporter(total: number, label: string = 'Progress') {
    return {
      start: () => console.log(`\n${label}...`),
