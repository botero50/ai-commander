...
import { createWorldState, createGameTime, createTick, createGameMap, createPosition, createPlayer, createAgentSnapshot, createAgent, createPlayerId, createResourcePool, AgentState as AgentStateEnum, } from '@ai-commander/domain';
/**
 ...
 */
export class FakeObservationProvider {
    ...
    async getWorldState() {
        if (!this.available) {
            throw new AdapterError('Observation provider is not available', AdapterErrorCode.ObservationUnavailable);
        }
    ...
    async getWorldStateAt(tick) {
        ...
        if (tick < 0 || tick >= this.worldHistory.length) {
            throw new AdapterError(`Tick ${tick} not available in history (length: ${this.worldHistory.length})`, AdapterErrorCode.ObservationFailed);
        }
    ...
    convertToWorldState(snapshot) {
        ...
        const positions = allUnits.map((u) => createPosition(`${u.x},${u.y}`, `(${u.x}, ${u.y})`));
...
//# sourceMappingURL=fake-observation-provider.js.map