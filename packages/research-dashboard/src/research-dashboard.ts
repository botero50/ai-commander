...
import type { HistoricalSnapshot, RatingSystem } from '@ai-commander/rating-system';
...
export class ResearchDashboard {
  ...
  private static generateModelComparisonTable(data: DashboardData): string {
    const modelStats = new Map<string, { wins: number; losses: number; draws: number; totalCost: number; }>();
  ...
  private static generateEloChart(data: DashboardData): string {
  ...
  private static generateWinRateChart(data: DashboardData): string {
