...
const commands = {
    tournament: {
        ...
        run: async (args) => {
            ...
            // Create brains
            const brains = await Promise.all(config.brains.map((brainConfig) => BrainManager.create(brainConfig)));
            console.log(`Running ${config.format} tournament with ${brains.length} brains...`);
            // Execute tournament
            const result = await TournamentEngine[config.format]({
                ...config,
                brains,
            });
            // Generate report
            ...
            const outfile = Array.isArray(args.output) ? args.output[0] : args.output || `tournament-${Date.now()}.${format === 'markdown' ? 'md' : format}`;
    ...
    match: {
        ...
        run: async (args) => {
            ...
            const redConfig = JSON.parse(args.red);
            const blueConfig = JSON.parse(args.blue);
            ...
            const replay = await MatchRunner.run({
                ...
                blueBrain,
                mapSeed: seed,
                maxTicks: parseInt(args.ticks) || 200,
                gameAdapterId: args.game,
            });
            const outfile = Array.isArray(args.output) ? args.output[0] : args.output || `match-${Date.now()}.json`;
    ...
    experiment: {
        ...
        run: async (args) => {
            ...
            const outfile = Array.isArray(args.output) ? args.output[0] : args.output || `experiment-${Date.now()}.md`;
    ...
    analyze: {
        ...
        run: async (args) => {
            ...
            const replay = JSON.parse(fs.readFileSync(args.replay, 'utf-8'));
            ...
            const outfile = Array.isArray(args.output) ? args.output[0] : args.output || `replay-${Date.now()}.html`;
    ...
    dashboard: {
        ...
        run: async (args) => {
            ...
            const tournaments = args.tournaments?.map((f) => JSON.parse(fs.readFileSync(f, 'utf-8'))) || [];
            const html = ResearchDashboard.generateHTML({
                tournaments,
                ratingHistory: [],
                selectedModels: args.models || [],
            });
            const outfile = Array.isArray(args.output) ? args.output[0] : args.output || `dashboard-${Date.now()}.html`;
            fs.writeFileSync(outfile, html);
