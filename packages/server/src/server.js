...
export class AICommanderServer {
    ...
    async handleTournament(req, res) {
        const body = await this.parseJSON(req);
        ...
        setImmediate(async () => {
            try {
                ...
                job.status = 'running';
                const brains = await Promise.all(body.brains.map((cfg) => BrainManager.create(cfg)));
    ...
    async handleGetTournament(req, res, id) {
        const job = this.tournamentJobs.get(id);
        if (!job) {
            res.writeHead(404, { 'Content-Type': 'application/json' });
    ...
    async handleGetResult(req, res, id) {
        const job = this.tournamentJobs.get(id);
        if (!job) {
            res.writeHead(404, { 'Content-Type': 'application/json' });
