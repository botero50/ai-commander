...
async function main() {
    ...
    try {
        ...
        if (!reachable) {
            console.log('✗ Cannot reach RL Interface at {}:{}'.replace('{}', RL_HOST).replace('{}', String(RL_PORT)));
        ...
        if (results.totalObservations > 0) {
            const avgPlayers = results.observations.reduce((s, o) => s + o.playersCount, 0) /
                results.observations.length;
            const avgEntities = results.observations.reduce((s, o) => s + o.entitiesCount, 0) /
                results.observations.length;
            const avgUnits = results.observations.reduce((s, o) => s + o.unitsCount, 0) /
                results.observations.length;
